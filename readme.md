...
A simulated VTOL drone that delivers medical items between hospitals. It carries things like blood samples, vaccines, and medicines from one hospital helipad to another through air. The drone takes off, flies on its own, and lands at the target hospital's helipad.
...
- **Companion Computer:**
  ...
  - `telemetry_streamer`: Sends live drone data over WebSockets
  - `web_server`: Runs the control panel and handles requests
- **Coordinates:** You enter Gazebo (ENU) coordinates on the dashboard. The web server converts these to GPS (WGS84) for the drone, which converts them again to local NED meters.
...
Optional — view the drone's 3D model:
...
4. Click **Send Mission**. The drone handles the rest.